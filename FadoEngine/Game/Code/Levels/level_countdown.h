...
internal void ParachuteMiniGame(FGameState* gameState, FGameInput* input, FGameControllerInput* controller)
{
    ...
    switch (level->miniGameState)
    {
        ...
        case MiniGame_Update:
        {
            shared->transforms.positions[level->parachute].y -= 5.0f * input->deltaTime;
...
internal void CoffeeMiniGame(FGameState* gameState, FGameInput* input, FGameControllerInput* controller)
{
    ...
    switch (level->miniGameState)
    {
        ...
        case MiniGame_Update:
        {
            ...
            f32 windPower = 0.625f;
            if (wind == WindDir_Left)
            {
                if (!Down(&controller->dpadLeft) || Down(&controller->dpadRight))
                {
                    warmth -= windPower * input->deltaTime;
                }
    ...
    switch (level->miniGameState)
    {
        ...
        case MiniGame_Update:
        {
            ...
            if (level->timer <= redTime)
            {
                ...
                if (Down(&controller->actionDown) || (goalValue <= 1.0f))
                {
            ...
            {
                if (Pressed(&controller->actionDown) || Released(&controller->actionDown))
                {
                ...
                else
                {
                    ...
                    goalValue = Clamp(goalValue, 0.0f, 100.0f);
                }

                shared->transforms.positions[level->redGreenBar] = { -5, -3, 0 };
                shared->transforms.scales[level->redGreenBar].y = goalValue;
                if (goalValue > 1.0f)
                {
...
internal void BasketballMiniGame(FGameState* gameState, FGameInput* input, FGameControllerInput* controller)
{
    ...
    case MiniGame_Update:
    {
        ...
        if (level->timer > 0.5f)
        {
            ...
            if (Pressed(&controller->actionDown) || Released(&controller->actionDown))
            {
                jumpStrength += 10.0f * input->deltaTime;
            }
            else
            {
            ...
            shared->transforms.scales[level->jumpBar].y = jumpStrength;
        ...
        else
        {
            ...
            if ((jumpStrength >= jumpGoal) && (jumpStrength <= (jumpGoal + 1.0f)))
            {
                SetClip(anim_0, 2);
                SetClip(anim_1, 0);
...
internal void Level_Countdown_HandleGameInput(FGameState* gameState, FGameInput* input, FGameControllerInput* controller)
{
        ...
        case MiniGame_End:
        {
            ...
            if (!playedYayBoo)
            {
                if (level->wonLastMiniGame)
                {
                    ...
                    if (level->particle_01 != INVALID_HANDLE)
                    {

                        FParticleEmitter* confetti = &gameState->shared->particles[level->particle_01];
                        confetti->hasBurst = false;
                        confetti->spawnRate = 0.0f;
                        confetti->count.start = FRangeF32{ 100.0f, 200.0f };
                        ...
                        if (level->particle_02 != INVALID_HANDLE)
                        {
                            confetti = &gameState->shared->particles[level->particle_02];
                            *confetti = gameState->shared->particles[level->particle_01];
    ...
    UpdateAllEmitters(gameState->shared->particles, input->deltaTime);
}
...
{
    FGameControllerInput* controller = input->controllers[1].isConnected ? &input->controllers[1] : &input->controllers[0];
    if (!controller->isConnected)
    {
        return;
    }
